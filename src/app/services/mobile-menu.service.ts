...
export class MobileMenuService {
    private isOpenSubject: BehaviorSubject<boolean> = new BehaviorSubject(
        false
    );

    isOpen$: Observable<boolean> = this.isOpenSubject.asObservable();

    onOpenPanel: EventEmitter<OpenPanelEvent> =
        new EventEmitter<OpenPanelEvent>();
    ...
    toggle(force?: boolean): void {
        const isOpen = force ?? !this.isOpenSubject.value;
