...
export class IndicatorComponent implements OnChanges, OnInit, OnDestroy {
    ...
    @HostBinding('class.indicator--trigger--click')
    get classIndicatorTriggerClick(): boolean {
        return this.trigger === 'click';
    }

    @HostBinding('class.indicator--trigger--hover')
    get classIndicatorTriggerHover(): boolean {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if ('link' in changes) {
            this.href = this.router
                .createUrlTree([this.link], { relativeTo: this.route })
                .toString();
        }
    }
    ...
    onClick(event: MouseEvent) {
        ...
        if (this.trigger !== 'click') {
            this.router
                .navigate([this.link], { relativeTo: this.route })
                .then();
        }
